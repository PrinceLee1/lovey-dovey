...
export default function CreateLobbyModal({ open, onClose, onCreated }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 grid place-items-center p-4">
      <div className="w-full max-w-lg rounded-3xl bg-white dark:bg-gray-900 shadow-xl p-5">
        <div className="flex items-center justify-between mb-3">
          <div className="text-xl font-semibold text-gray-900 dark:text-gray-100">Create Lobby</div>
          <button onClick={onClose} className="rounded-lg border dark:border-gray-700 dark:text-gray-200 px-2 py-1 text-sm hover:bg-gray-50 dark:hover:bg-gray-800"><X className="w-4 h-4"/></button>
        </div>

        {err && <div className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-xl px-3 py-2 mb-3">{err}</div>}

        <div className="space-y-3">
          <div>
            <div className="text-sm text-gray-700 dark:text-gray-300 mb-1">Lobby name</div>
            <input value={name} onChange={(e)=>setName(e.target.value)} className="w-full rounded-xl border dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 px-3 py-2 outline-none focus:ring-2 focus:ring-fuchsia-500" placeholder="Game Night"/>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="text-sm text-gray-700 dark:text-gray-300 mb-1">Max players</div>
          ...
          <div>
            <div className="text-sm text-gray-700 dark:text-gray-300 mb-1">Privacy</div>
            <select
              value={privacy}
          ...
          <div>
            <div className="text-sm text-gray-700 dark:text-gray-300 mb-1">Start time</div>
