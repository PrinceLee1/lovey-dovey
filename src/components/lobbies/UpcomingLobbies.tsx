...
export default function UpcomingLobbies({ variants }: { variants: MotionProps }) {
  ...
  return (
    <motion.div {...variants} className="rounded-3xl bg-white dark:bg-gray-900 shadow-xl border border-rose-100 dark:border-gray-800 p-6">
      <div className="font-display font-semibold text-gray-900 dark:text-gray-100 mb-4">Upcoming Lobbies</div>

      {err && (
        <div className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-xl px-3 py-2 mb-3">
          {err}
        </div>
      )}
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="animate-pulse border dark:border-gray-800 rounded-2xl p-3">
        ...
        <div className="text-sm text-gray-500 dark:text-gray-400">No lobbies yet. Be the first to create one!</div>
      ) : (
        <div className="space-y-3">
          {list.map((l) => {
            ...
            return (
              <div key={l.id} className="border dark:border-gray-800 rounded-2xl p-3 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-3 mt-2">
                ...
                {mineIds.has(l.id) ? (
                  ...
                  <button
                    ...
                    className="text-sm px-2 py-1.5 rounded-lg border dark:border-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 -mt-2"
                  >
                  ...
                  l.host_id == user?.id && (
                    <div title="Delete Lobby" className="-mt-5">
                      <button
                        ...
                        className="text-sm px-3 py-1.5 rounded-lg border dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800"
                      >
