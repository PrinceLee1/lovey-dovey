...
export default function TruthDarePlus({ couple, category = "Spicy", onFinish }: Props) {
  ...
  return (
    <div className="space-y-5">
      {err && <div className="text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/40 border border-red-100 dark:border-red-900 rounded-xl px-3 py-2">{err}</div>}

      {/* ── Header stats ──────────────────────────────────────────────────── */}
      <div className="grid grid-cols-4 gap-2 text-center">
        <div className="rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-100 dark:border-rose-900 p-2">
          <div className="text-xs text-gray-500 dark:text-gray-400">Round</div>
          <div className="font-bold text-gray-900 dark:text-gray-100">{round}</div>
        </div>
        <div className="rounded-2xl bg-fuchsia-50 dark:bg-fuchsia-950/40 border border-fuchsia-100 dark:border-fuchsia-900 p-2">
          <div className="text-xs text-gray-500 dark:text-gray-400">Streak</div>
          <div className="font-bold text-fuchsia-700 dark:text-fuchsia-300 flex items-center justify-center gap-1">
            <Flame className="w-3 h-3" />{streak}
          </div>
        </div>
        <div className="rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-100 dark:border-amber-900 p-2">
          <div className="text-xs text-gray-500 dark:text-gray-400">Multiplier</div>
          <div className="font-bold text-amber-600 dark:text-amber-300">{multiplier}×</div>
        </div>
        <div className="rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-900 p-2">
          <div className="text-xs text-gray-500 dark:text-gray-400">XP</div>
          <div className="font-bold text-emerald-700 dark:text-emerald-300">{totalXp}</div>
        </div>
      </div>

      {/* ── Player turn ───────────────────────────────────────────────────── */}
      <div className="flex items-center justify-between text-sm">
        <span className="text-gray-600 dark:text-gray-400">Turn: <b className="text-gray-900 dark:text-gray-100">{players[pIdx]}</b></span>
        <div className="flex items-center gap-2">
          <span className="text-xs px-2 py-1 rounded-full border dark:border-gray-700">Skips: {skips}</span>
          {multiplier >= 2 && (
            <span className="text-xs px-2 py-1 rounded-full bg-amber-100 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-900 flex items-center gap-1">
      ...
      <AnimatePresence mode="wait">
            ...
            <div className="rounded-2xl border border-rose-200 dark:border-rose-900 bg-rose-50 dark:bg-rose-950/40 p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-rose-700 dark:text-rose-300">TRUTH</span>
                </div>
                {/* Timer */}
                <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-mono font-bold ${
                  timeLeft <= 10 ? "bg-red-100 dark:bg-red-950/40 text-red-700 dark:text-red-400 animate-pulse" : "bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border dark:border-gray-700"
                }`}>
              ...
              {/* Timer bar */}
              <div className="h-1.5 w-full bg-gray-200 dark:bg-gray-800 rounded-full mb-4 overflow-hidden">
                <motion.div
              ...
              <p className="text-gray-900 dark:text-gray-100 font-medium text-base leading-relaxed">{prompt}</p>
              ...
              <div className="mt-4 flex items-center gap-2">
                ...
                <button
                  onClick={skipPrompt}
                  disabled={skips <= 0}
                  className="rounded-xl px-3 py-2 border dark:border-gray-700 text-sm hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 inline-flex items-center gap-1"
                >
        ...
        {phase === "dare" && (
          <motion.div key="dare" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <div className="rounded-2xl border border-fuchsia-200 dark:border-fuchsia-900 bg-fuchsia-50 dark:bg-fuchsia-950/40 p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <Sparkles className="w-5 h-5 text-fuchsia-600" />
                  <span className="font-semibold text-fuchsia-700 dark:text-fuchsia-300">DARE</span>
                </div>
                <span className="text-xs px-2 py-1 rounded-full bg-white dark:bg-gray-800 border dark:border-gray-700 text-gray-600 dark:text-gray-400">No time limit</span>
              </div>
              ...
              <div className="mt-4 rounded-xl border border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/40 p-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-amber-700 dark:text-amber-400">60% chance to double XP — or get nothing</div>
                  </div>
                  <button
                    ...
                    className={`rounded-xl px-3 py-1.5 text-xs font-semibold border transition ${
                      doubleDown
                        ...
                        : "bg-white dark:bg-gray-800 text-amber-700 dark:text-amber-300 border-amber-300 dark:border-amber-800 hover:bg-amber-50 dark:hover:bg-amber-900/40"
                    }`}
                  >
                    {doubleDown ? "ON 🎲" : "OFF"}
                  </button>
        ...
        {phase === "challenge" && (
          <motion.div key="challenge" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}>
            <div className="rounded-2xl border-2 border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl bg-white dark:bg-gray-800 border dark:border-gray-700 p-3">
                  <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">{players[0]}</div>
                  <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{prompt}</div>
                </div>
                <div className="rounded-xl bg-white dark:bg-gray-800 border dark:border-gray-700 p-3">
                  <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">{players[1]}</div>
                  <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{prompt2}</div>
                </div>
              </div>
              <div className="text-xs text-amber-700 dark:text-amber-400">Both must complete for +{baseXp.challenge * multiplier} XP each</div>
              <button
      ...
      <div className="flex items-center justify-between pt-1">
        <button onClick={restart} className="text-sm text-gray-500 dark:text-gray-400 inline-flex items-center gap-1 hover:text-gray-800 dark:hover:text-gray-200">
